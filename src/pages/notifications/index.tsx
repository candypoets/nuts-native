...
function Page() {
	...
	const { events, replies, mentions, reactions, reposts, loading } = useNotifications(pubkey);
	const { markAllRead } = useNotificationStore(pubkey);
		...
		<PageShell title="Notifications">
			...
			<scroll-view className="flex-1">
				...
				{activeEvents.map((event) => {
					const type = categorizeEvent(event, pubkey) || 'mention';
